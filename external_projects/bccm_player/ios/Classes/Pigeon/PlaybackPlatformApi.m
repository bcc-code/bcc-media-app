...
void PlaybackPlatformPigeonSetup(id<FlutterBinaryMessenger> binaryMessenger, NSObject<PlaybackPlatformPigeon> *api) {
  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        initWithName:@"dev.flutter.pigeon.PlaybackPlatformPigeon.getPlaybackSpeed"
        binaryMessenger:binaryMessenger
        codec:PlaybackPlatformPigeonGetCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(getPlaybackSpeed:completion:)], @"PlaybackPlatformPigeon api (%@) doesn't respond to @selector(getPlaybackSpeed:completion:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray *args = message;
        NSString *arg_playerId = GetNullableObjectAtIndex(args, 0);
        [api getPlaybackSpeed:arg_playerId completion:^(NSNumber *_Nullable output, FlutterError *_Nullable error) {
          callback(wrapResult(output, error));
        }];
      }];
    } else {
  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        ...
        codec:PlaybackPlatformPigeonGetCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(setPlaybackSpeed:playbackSpeed:completion:)], @"PlaybackPlatformPigeon api (%@) doesn't respond to @selector(setPlaybackSpeed:playbackSpeed:completion:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray *args = message;
        NSString *arg_playerId = GetNullableObjectAtIndex(args, 0);
        NSNumber *arg_playbackSpeed = GetNullableObjectAtIndex(args, 1);
        [api setPlaybackSpeed:arg_playerId playbackSpeed:arg_playbackSpeed completion:^(FlutterError *_Nullable error) {
          callback(wrapResult(nil, error));
        }];
      }];
    } else {
