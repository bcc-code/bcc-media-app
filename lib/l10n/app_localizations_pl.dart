// ignore: unused_import
import 'package:intl/intl.dart' as intl;
import 'app_localizations.dart';

// ignore_for_file: type=lint

/// The translations for Polish (`pl`).
class SPl extends S {
  SPl([String locale = 'pl']) : super(locale);

  @override
  String get loadingContent => 'Ładuję zawartość';

  @override
  String get signInTooltip => 'Zaloguj się, by móc oglądać więcej';

  @override
  String get skipToPublicContent => 'Pomiń, by obejrzeć publiczną zawartość';

  @override
  String get minutesShort => 'min';

  @override
  String get calendarThisWeek => 'W tym tygodniu';

  @override
  String get calendarWeek => 'tydzień';

  @override
  String get close => 'Zamknij';

  @override
  String get loading => 'Ładuję';

  @override
  String get loginPageDisplay1 => 'Najpotężniejsze poselstwo na świecie';

  @override
  String get loginPageDisplay2 =>
      'Oglądaj seriale, produkcje i filmy oparte na chrześcijańskich wartościach';

  @override
  String get kidsOnboardingTitle =>
      'Sprawiamy, że przesłanie Biblii staje się żywe!';

  @override
  String get kidsOnboardingSecondaryTitle =>
      'Naszą wizją jest, aby dzieci miały Jezusa za swojego najlepszego przyjaciela – do końca życia!';

  @override
  String get loginPageDisplay3 => 'Wyprodukowane przez BCC Media';

  @override
  String get logOutButton => 'Wyloguj się';

  @override
  String get profileTab => 'Profil';

  @override
  String get faq => 'Często zadawane pytania';

  @override
  String get bccMediaCenter =>
      'BCC Media to fundacja medialna w federacji BCC, która zajmuje się produkcją i dystrybucją treści o charakterze pozytywnym i chrześcijańskim.';

  @override
  String get signInButton => 'Zaloguj się';

  @override
  String get signInOrSignUp => 'Zarejestruj się / Zaloguj się';

  @override
  String get signUpButton => 'Zarejestruj się teraz';

  @override
  String get signUpTitle => 'Zarejestruj się';

  @override
  String get signUpDescription =>
      'Uzyskaj dostęp do większej ilości treści i lepszych funkcji, tworząc konto.';

  @override
  String get signUpAgreePrivacyPolicy =>
      'Oświadczam, że mam ukończone 13 lat i zgadzam się z <a>Polityką prywatności</a> i <a>Warunkami użytkowania</a>.';

  @override
  String get setPassword => 'Ustaw hasło';

  @override
  String get tryAgainButton => 'Spróbuj ponownie';

  @override
  String get loggedOutTitle => 'Wylogowany';

  @override
  String get loggedOutMessage => 'Zostałeś wylogowany. Zaloguj się ponownie.';

  @override
  String get makeDonation => 'Przekaż darowiznę';

  @override
  String get mostWatched => 'Najpopularniejsze';

  @override
  String get mostRecent => 'Ostatnio dodane';

  @override
  String get latelyPublished => 'Ostatnio opublikowane';

  @override
  String get anErrorOccurred => 'Wystąpił błąd';

  @override
  String get ok => 'OK';

  @override
  String get open => 'Otwórz';

  @override
  String get loginFailedCheckNetwork =>
      'Logowanie nie powiodło się Sprawdź połączenie z Internetem i spróbuj ponownie.';

  @override
  String get loginFailedTryAgain =>
      'Logowanie nie powiodło się Spróbuj ponownie później.';

  @override
  String get soundLanguage => 'Język video';

  @override
  String get subtitleLanguage => 'Język napisów';

  @override
  String get appLanguage => 'Język aplikacji';

  @override
  String get checkNetwork =>
      'Sprawdź połączenie z Internetem i spróbuj ponownie.';

  @override
  String get tryAgain => 'Spróbuj ponownie później.';

  @override
  String get cantPlayVideo => 'Nie możemy teraz odtworzyć tego video.';

  @override
  String get about => 'O nas';

  @override
  String get version => 'Wersja';

  @override
  String get contactSupport => 'Zgłoś problem';

  @override
  String get contactIssueHint => 'Opisz problem';

  @override
  String get contactUs => 'Kontakt';

  @override
  String get deviceInfo => 'Informacje o urządzeniu';

  @override
  String get contactIncludeDeviceInfo =>
      'Dołącz informacje o urządzeniu, aby uzyskać lepsze wsparcie.';

  @override
  String get contactSeeData => 'Kliknij, aby zobaczyć dane.';

  @override
  String get contactName => 'Imię';

  @override
  String get contactNameHint => 'Wpisz swoje imię';

  @override
  String get contactEmail => 'Twój adres e-mail';

  @override
  String get contactEmailHint => 'Wpisz swój adres e-mail';

  @override
  String get contactMessage => 'Wiadomość';

  @override
  String get contactMessageHint => 'Wpisz swoją wiadomość';

  @override
  String get deviceInfoDevice => 'Urządzenie';

  @override
  String get deviceInfoEnvOverride => 'Nadpisanie środowiska';

  @override
  String get deviceInfoManufacturer => 'Producent';

  @override
  String get deviceInfoOs => 'System operacyjny';

  @override
  String get deviceInfoScreenSize => 'Rozmiar ekranu';

  @override
  String get deviceInfoAppVer => 'Wersja aplikacji';

  @override
  String get deviceInfoUserId => 'Identyfikator użytkownika';

  @override
  String get deviceInfoPageDesc =>
      'Twoja wiadomość będzie zawierać te informacje, aby pomóc nam zapewnić lepsze wsparcie.';

  @override
  String get userVoice => 'Zaproponuj funkcję';

  @override
  String get cancel => 'Anuluj';

  @override
  String get cancelConfirmation => 'Czy na pewno chcesz anulować?';

  @override
  String get cancelConfirmationDescription =>
      'Wszelkie niezapisane zmiany zostaną utracone.';

  @override
  String get goBack => 'Wróć';

  @override
  String get yesCancel => 'Tak, anuluj';

  @override
  String get done => 'Gotowe';

  @override
  String get playerSettings => 'Ustawienia odtwarzania';

  @override
  String get settings => 'Ustawienia';

  @override
  String get none => 'Bez napisów';

  @override
  String get nextEpisode => 'Kolejny Odcinek';

  @override
  String get auto => 'Auto';

  @override
  String get subtitles => 'Napisy';

  @override
  String get audio => 'Dźwięk';

  @override
  String get quality => 'Jakość';

  @override
  String get seasons => 'Sezony';

  @override
  String get episodes => 'Odcinki';

  @override
  String get search => 'Szukaj';

  @override
  String get sendFeedback => 'Wyślij opinię';

  @override
  String get noResults => 'Brak wyników';

  @override
  String get doYouWantCast => 'Czy odtworzyć na Chromecast?';

  @override
  String get play => 'Odtwarzaj';

  @override
  String get pause => 'Pauza';

  @override
  String get castQueue => 'Chromecast';

  @override
  String get options => 'Opcje';

  @override
  String get remove => 'Usuń';

  @override
  String get queueLimitTitle => 'Limit';

  @override
  String queueLimitMessage(int elementCount) {
    return 'Nie możesz mieć więcej niż $elementCount elementów w kolejce.';
  }

  @override
  String get recentSearches => 'Ostatnie wyszukiwania';

  @override
  String get tVLogin => 'Logowanie na telewizorze';

  @override
  String get tVLoginSuccessful =>
      'Logowanie przebiegło pomyślnie. Potwierdź logowanie na telewizorze.';

  @override
  String get seeMore => 'Więcej';

  @override
  String get seeAll => 'Wszystkie';

  @override
  String get series => 'Serie';

  @override
  String get homeTab => 'Odkrywaj';

  @override
  String get sortBy => 'Sortowanie';

  @override
  String get videoQuality => 'Jakość video';

  @override
  String get low => 'Niska';

  @override
  String get medium => 'Średnia';

  @override
  String get high => 'Wysoka';

  @override
  String get emptySearch =>
      'Możesz przeszukiwać całą zawartość BCC Media. Serie, filmy i odcinki.';

  @override
  String get back => 'Wstecz';

  @override
  String get seasonLetter => 'S';

  @override
  String get episodeLetter => 'O';

  @override
  String get atoZ => 'A-Z';

  @override
  String get ztoA => 'Z-A';

  @override
  String get shortFilms => 'Krótkie filmy';

  @override
  String get videos => 'Filmy';

  @override
  String get moreInfo => 'Więcej informacji';

  @override
  String get info => 'Informacje';

  @override
  String get episode => 'Odcinek';

  @override
  String get takePhoto => 'Zrób zdjęcie';

  @override
  String get pickPhoto => 'Wybierz z galerii';

  @override
  String get uploadFileTitle => 'Wyślij plik';

  @override
  String get uploadFileMessage => 'Czy chcesz wysłać ten plik?';

  @override
  String get uploading => 'Wysyłanie';

  @override
  String get uploadFail => 'Wysyłanie nie powiodło się';

  @override
  String charactersLimit(int charsLeft, int charsTotal) {
    return 'Pozostało $charsLeft z $charsTotal znaków';
  }

  @override
  String get thankYou => 'Dziękujemy!';

  @override
  String get thankYouForContribution => 'Dziękujemy za Twój wkład!';

  @override
  String get next => 'Następne';

  @override
  String get add => 'Dodaj';

  @override
  String get change => 'Zmień';

  @override
  String get writeHere => 'Pisz tutaj';

  @override
  String get feed => 'Wpisy';

  @override
  String get information => 'Informacje';

  @override
  String get question => 'Pytanie';

  @override
  String get now => 'Teraz';

  @override
  String get debugInfoExplanation =>
      'Ta informacja jest nam potrzebna, byśmy mogli lepiej zrozumieć problem.';

  @override
  String get send => 'Wyślij';

  @override
  String get sending => 'Wysyłam';

  @override
  String get thankYouSupportTitle => 'Dziękujemy za informację!';

  @override
  String get thankYouSupportDescription =>
      'Doceniamy wszelkie informacje zwrotne i odniesiemy się do tego problemu tak szybko, jak to możliwe.';

  @override
  String get sendFail => 'Nie udało się wysłać wiadomości';

  @override
  String get sendFailDescription =>
      'Coś poszło nie tak. Sprawdź połączenie internetowe i spróbuj ponownie.';

  @override
  String get sendSuccessDescription => 'Doceniamy twój feedback.';

  @override
  String get today => 'Dzisiaj';

  @override
  String get expired => 'Wygasło';

  @override
  String availableFrom(String date) {
    return 'Dostępny $date';
  }

  @override
  String get submit => 'Wyślij';

  @override
  String get wellDone => 'Dobra robota!';

  @override
  String get editPicture => 'Edytuj zdjęcie';

  @override
  String get profilePicture => 'Zdjęcie profilowe';

  @override
  String timezoneInformation(String timeZone) {
    return 'Godziny w programie TV są przedstawione w twojej strefie czasowej: $timeZone';
  }

  @override
  String get share => 'Udostępnij';

  @override
  String get showMore => 'Pokaż więcej';

  @override
  String get showLess => 'Pokaż mniej';

  @override
  String get comingSoon => 'Dostępne wkrótce';

  @override
  String get newEpisode => 'Nowy';

  @override
  String get tomorrow => 'Jutro';

  @override
  String get readBy => 'Lektor';

  @override
  String get broadcasts => 'Transmisje';

  @override
  String get calendar => 'Kalendarz';

  @override
  String get noEvents => 'Brak zaplanowanych transmisji';

  @override
  String get appUpdateTitle => 'Wymagana aktualizacja';

  @override
  String get appUpdateRequest =>
      'Zainstaluj najnowszą wersję aplikacji by korzystać z najnowszych funkcjonalności i poprawek.';

  @override
  String get appUpdateAccepted => 'Zaktualizuj';

  @override
  String get copy => 'Kopiuj';

  @override
  String get reload => 'Załaduj ponownie';

  @override
  String get fAQ => 'FAQ';

  @override
  String get shareStart => 'Udostępnij od początku';

  @override
  String shareTime(String time) {
    return 'Udostępnij od $time';
  }

  @override
  String get loadContentError => 'Nie udało się załadować treści';

  @override
  String get privacyPolicy => 'Polityka prywatności';

  @override
  String get termsOfUse => 'Warunki użytkowania';

  @override
  String get categoriesTab => 'Kategorie';

  @override
  String get seriesTab => 'Serie';

  @override
  String get filmsTab => 'Filmy';

  @override
  String get eventsTab => 'Wydarzenia';

  @override
  String get audioLanguage => 'Język dźwięku';

  @override
  String get subtitlesLanguage => 'Język napisów';

  @override
  String get removeFromRecentSearches =>
      'Czy chcesz usunąć ten element z ostatnio wyszukanych?';

  @override
  String get noTransmissionToday => 'Brak zaplanowanych transmisji na dzisiaj';

  @override
  String get featured => 'Polecane';

  @override
  String get errorTryAgain => 'Coś poszło nie tak, spróbuj ponownie później.';

  @override
  String get programsSection => 'Programy';

  @override
  String get noPlannedEvents => 'Brak wydarzeń w tym dniu';

  @override
  String get watchNow => 'Oglądaj teraz';

  @override
  String get exploreContent => 'Odkryj zawartość';

  @override
  String get explorePublicContent => 'Przeglądaj opublikowane treści';

  @override
  String get startExploring => 'Rozpocznij odkrywanie';

  @override
  String get taskCompleted => 'ukończone zadanie';

  @override
  String get tasksCompleted => 'ukończonych zadań';

  @override
  String get studyCompleteTaskDescription =>
      'Wykonuj zadania i odkryj powiązane materiały';

  @override
  String get discoverMore => 'Odkryj więcej';

  @override
  String get completePreviousTasks =>
      'Ukończ poprzednie zadania, by odblokować';

  @override
  String get studyDiscoverMoreDescription =>
      'Znajdź więcej inspiracji i wiedzy wśród powiązanych materiałów';

  @override
  String get watchAgain => 'Obejrzyj ponownie';

  @override
  String get continueToTaskIn => 'Przejdź do zadania za';

  @override
  String get continueToTasksIn => 'Przejdź do zadań za';

  @override
  String get studyLessonCompleted => 'lekcja ukończona';

  @override
  String get studyLessonsCompleted => 'lekcji ukończonych';

  @override
  String get continueStudy => 'Kontynuuj naukę';

  @override
  String get startStudy => 'Rozpocznij naukę';

  @override
  String get videoCompletedText => 'Gratulacje! Obejrzałeś filmik.';

  @override
  String get achievementFirstDescription =>
      'Gratulacje! Obejrzałeś filmik i zdobyłeś pierwszą nagrodę za postęp w projekcie.';

  @override
  String achievementStudyTopic(String name) {
    return 'Gratulacje! Ukończyłeś lekcję w temacie $name';
  }

  @override
  String videoCount(int count) {
    return '$count filmy/-ów';
  }

  @override
  String taskCount(int count) {
    return '$count zadań/-nia';
  }

  @override
  String posterCount(int count) {
    return '$count plakaty/-ów';
  }

  @override
  String resourceCount(int count) {
    return '$count materiały/-ów';
  }

  @override
  String get continueButton => 'Kontynuuj';

  @override
  String get registerButton => 'Zarejestruj się';

  @override
  String get achievements => 'Osiągnięcia';

  @override
  String passwordConditionLength(int n) {
    return 'Minimum $n znaków';
  }

  @override
  String get passwordConditionComplexity =>
      'Musi zawierać małe i wielkie litery oraz cyfry.';

  @override
  String get oops => 'Ups!';

  @override
  String get dontHaveAccess => 'Nie masz dostępu do tej zawartości.';

  @override
  String get notFound =>
      'Wygląda na to, że zawartość, której szukasz, nie istnieje.';

  @override
  String get mustBeLoggedIn =>
      'Musisz być zalogowany, by obejrzeć tę zawartość';

  @override
  String get plentyOfOtherContent =>
      'Nic się nie martw, mamy dla ciebie mnóstwo innych materiałów!';

  @override
  String get notAchieved => 'Nie osiągnięte';

  @override
  String get details => 'Szczegóły';

  @override
  String get myList => 'Moja lista';

  @override
  String get didYouKnowTitle => 'Czy wiedziałeś?';

  @override
  String get didYouKnowContent =>
      'Możesz dodać swoje ulubione programy do \"Mojej listy\", klikając w serduszko.';

  @override
  String get selectSeason => 'Wybierz sezon';

  @override
  String get deleteMyAccount => 'Usuń konto';

  @override
  String get iAmSureIWantToDelete =>
      'Jestem pewien, że chcę usunąć moje konto i dane związane z aplikacją BCC Media.';

  @override
  String get areYouSure => 'Czy jesteś pewien?';

  @override
  String get youCanContactUsAt => 'Możesz się z nami skontaktować pod adresem';

  @override
  String get biblekidsAbout =>
      'Bible Kids to inicjatywa BCC Media, fundacji medialnej w ramach Federacji BCC, która produkuje i rozpowszechnia treści o pozytywnych i chrześcijańskich wartościach.';

  @override
  String get askYourParents => 'Zapytaj rodziców';

  @override
  String get whatIsMath => 'Ile to';

  @override
  String get answerLabel => 'Odpowiedź';

  @override
  String get checkAnswerButton => 'Sprawdź odpowiedź';

  @override
  String get videoOnlyAccessibleToLoggedIn =>
      'To wideo jest dostępne wyłącznie dla zalogowanych użytkowników.';

  @override
  String get verifyYourAccount => 'Zweryfikuj swoje konto';

  @override
  String get weHaveSentAnEmailTo => 'Wysłaliśmy wiadomość e-mail na';

  @override
  String get clickTheLinkToVerify =>
      'Kliknij w link w wiadomości e-mail, aby zweryfikować swoje konto.';

  @override
  String get openEmailApp => 'Otwórz aplikację poczty e-mail';

  @override
  String get resendEmail => 'Wyślij e-mail ponownie';

  @override
  String get noMailAppsDescription =>
      'Brak zainstalowanych aplikacji poczty e-mail';

  @override
  String get verificationEmailSent => 'Wysłano wiadomość e-mail z weryfikacją';

  @override
  String get ifYouDidntReceiveVerificationEmail =>
      'Jeśli nadal nie otrzymałeś wiadomości e-mail, skontaktuj się z pomocą techniczną pod adresem';

  @override
  String get accountCreated => 'Utworzono konto';

  @override
  String get youCanNowUseYourAccount =>
      'Możesz teraz korzystać ze swojego konta i odkrywać różne budujące treści.';

  @override
  String get youCanNowLogInToYourAccount =>
      'Możesz teraz zalogować się na swoje konto i odkrywać różne budujące treści.';

  @override
  String get accountDeletionExplanation =>
      'Zamierzasz poprosić o usunięcie swojego konta w BCC Media i wszystkich danych związanych z tą aplikacją.';

  @override
  String get accountDeletionWarning =>
      'Jest to działanie, którego nie można cofnąć.';

  @override
  String accountDeletionTimeframe(int x) {
    return 'Usunięcie konta następuje zazwyczaj w ciągu $x dni roboczych.';
  }

  @override
  String get accountDeletionTitle => 'Usuń konto';

  @override
  String accountDeletionSuccess(String email) {
    return 'Pomyślnie zażądano usunięcia konta. Otrzymasz wiadomość e-mail na adres $email. Aby potwierdzić swoją tożsamość, odpowiedz na tę wiadomość e-mail, aby kontynuować usunięcie konta.';
  }

  @override
  String get gamesTab => 'Gry';

  @override
  String get playGame => 'Zagraj w grę';

  @override
  String get tvScanQr => 'Zeskanuj kod i zaloguj się za pomocą telefonu';

  @override
  String tvEnterCode(String url, String code) {
    return 'Lub przejdź do: $url i wprowadź ten kod: $code';
  }

  @override
  String get chapters => 'Rozdziały';

  @override
  String get removeWatchProgress => 'Zacznij od początku';

  @override
  String get showDescription => 'Pokaż opis';

  @override
  String get releaseDate => 'Data premiery';

  @override
  String get availableTo => 'Dostępne do';

  @override
  String get ageRating => 'Ograniczenie wiekowe';

  @override
  String get ageRatingAll => 'Wszyscy';

  @override
  String get offline => 'Offline';

  @override
  String get downloadButton => 'Pobierz';

  @override
  String audioNotAvailableInLanguage(String lang) {
    return 'Dźwięk niedostępny po $lang';
  }

  @override
  String get notEnoughAvailableSpace =>
      'Za mało dostępnego miejsca na urządzeniu';

  @override
  String get considerDeletingOtherVideos =>
      'Rozważ usunięcie innych filmów pobranych w aplikacji.';

  @override
  String theVideoFileSizeIs(String size) {
    return 'Rozmiar pliku wideo to $size.';
  }

  @override
  String get gotIt => 'Rozumiem';

  @override
  String get saveYourFavorites => 'Zapisz swoje ulubione filmy';

  @override
  String get saveYourFavoritesDescription =>
      'Stuknij ikonę serca na filmie, aby zapisać go w swoim profilu.';

  @override
  String get yourFavorites => 'Twoje ulubione';

  @override
  String get downloaded => 'Pobrane';

  @override
  String get saveVideosForOfflineViewing =>
      'Zapisuj filmy do oglądania w trybie offline';

  @override
  String get tapOnTheDownloadIcon =>
      'Stuknij ikonę pobierania na filmie, aby go pobrać i odtwarzać w trybie offline.';

  @override
  String get removeOfflineVideo => 'Usunąć wideo offline?';

  @override
  String get removeOfflineVideoDescription =>
      'Po usunięciu tego filmu nie będzie on już dostępny bez połączenia z siecią.';

  @override
  String get removeFromDeviceButton => 'Usuń z urządzenia';

  @override
  String get keepVideoButton => 'Zachowaj wideo';

  @override
  String get noInternetConnection => 'Brak połączenia z Internetem';

  @override
  String get noInternetConnectionDescription =>
      'Połącz się z Internetem, aby uzyskać dostęp do wszystkich filmów.';

  @override
  String get noInternetButDontWorry =>
      'Nie masz połączenia internetowego, ale nie martw się.';

  @override
  String get hereAreAllTheVideosYouDownloaded =>
      'Oto wszystkie filmy, które pobrałeś na swoje urządzenie.';

  @override
  String get connectToExplore =>
      'Połącz się z internetem, aby przeglądać wszystkie filmy.';

  @override
  String get language => 'Język';

  @override
  String estimatedFileSize(String size) {
    return 'Szacowany rozmiar pliku: $size';
  }

  @override
  String get downloadVideo => 'Pobierz wideo';

  @override
  String get noSubtitles => 'Brak napisów';

  @override
  String get subtitlesIncluded => 'Dostępne napisy';

  @override
  String get allSubtitlesWillBeAvailableOffline =>
      'Wszystkie napisy będą dostępne w trybie offline';

  @override
  String get downloading => 'Pobieranie';

  @override
  String get failed => 'Niepowodzenie';

  @override
  String get finished => 'Zakończono';

  @override
  String get paused => 'Wstrzymano';

  @override
  String get queued => 'W kolejce';

  @override
  String get removing => 'Usuwanie';

  @override
  String expiresIn(String time) {
    return 'Wygasa za $time';
  }

  @override
  String get availableOffline => 'Dostępne offline';

  @override
  String get technicalDetails => 'Szczegóły techniczne';

  @override
  String get contentLanguage => 'Język zawartości';

  @override
  String get contentLanguageShort => 'Treść';

  @override
  String get appLanguageShort => 'Aplikacja';

  @override
  String get playRandom => 'Odtwarzanie losowe';

  @override
  String get addLanguage => 'Dodaj język';

  @override
  String get preferredLanguages => 'Preferowane języki';

  @override
  String get contentLanguageExplanation =>
      'Zmień kolejność języków, aby dopasować język dźwięku i napisów do swoich preferencji.';

  @override
  String get otherLanguages => 'Inne języki';

  @override
  String get select => 'Wybierz';

  @override
  String get supportBibleKids => 'Wspieraj Bible Kids';

  @override
  String get donateViaPaypal => 'Przekaż darowiznę przez PayPal';

  @override
  String get shortsTab => 'Shorts';

  @override
  String get autoGenerated => 'Generowane automatycznie';

  @override
  String get shortsTranscriptionDisclaimer =>
      'Transkrypcje są generowane automatycznie i mogą zawierać błędy.';

  @override
  String get beta => 'Beta';

  @override
  String get betaExplanation =>
      'Ta funkcja dostępna jest w wersji beta i obecnie nie jest dostępna dla wszystkich użytkowników. Zgłoś wszelkie problemy lub opinie za pomocą przycisku \"Kontakt\" w ustawieniach.';

  @override
  String get likedShorts => 'Polubione shortsy';

  @override
  String get saveYourFavoriteShorts => 'Zapisz swoje ulubione shortsy';

  @override
  String get featureNotAvailableYet =>
      'Ta funkcja nie jest jeszcze dostępna na Twoim urządzeniu. Spróbuj ponownie później.';

  @override
  String get newFeature => 'Nowa funkcja';

  @override
  String get shortsFeatureDescription =>
      'Odkrywaj krótkie i mocne fragmenty i przejdź od razu do pełnego odcinka.';

  @override
  String get likedAndSaved => 'Polubione i zapisane';

  @override
  String get shortsLikedAndSavedDescription =>
      'Wszystkie polubione filmy można znaleźć w swoim profilu.';

  @override
  String get excerptFrom => 'Fragment z';

  @override
  String get downloadedVideosGuideDescription =>
      'Pobrane filmy można znaleźć w \"Profilu\".';

  @override
  String get openStore => 'Otwórz';

  @override
  String get thisVideoCantBeDownloaded => 'Tego filmu nie można pobrać.';

  @override
  String get videoLanguage => 'Język wideo';

  @override
  String get videoTextLanguage => 'Język tekstu na wideo';

  @override
  String get original => 'Oryginalny';

  @override
  String get openSettingsToChangeVideoLang =>
      'Otwórz ustawienia, aby zmienić język tekstu wideo';

  @override
  String get somethingWentWrong => 'Coś poszło nie tak';

  @override
  String get all => 'Wszystkie';

  @override
  String countItems(int x) {
    return '$x pozycji';
  }

  @override
  String skipTo(String chapter) {
    return 'Przejdź do $chapter';
  }

  @override
  String get bmmBibleStudyTitle => 'Twoje studium Biblii';

  @override
  String bmmBibleStudyPoints(int points) {
    return '$points punktów';
  }

  @override
  String get answerTheQuiz => 'Odpowiedz na quiz';

  @override
  String get studyAnswerTheQuizDescription =>
      'Zdobywaj punkty, udzielając poprawnych odpowiedzi';

  @override
  String studySomeAnswersCorrect(int correct, int total) {
    return 'Masz $correct na $total poprawnych odpowiedzi';
  }

  @override
  String get studyAllAnswersCorrect => 'Wszystkie odpowiedzi są poprawne';

  @override
  String get studyNoAnswersCorrect => 'Żadna odpowiedź nie jest poprawna';

  @override
  String get studyAllAnswersCorrectDescription => 'Dobra robota!';

  @override
  String get studyNoAnswersCorrectDescription =>
      'Następnym razem słuchaj uważniej!';

  @override
  String get studySomeAnswersCorrectDescription =>
      'Dobra robota! Ale wciąż masz pole do poprawy.';

  @override
  String get studyAllQuestionsAnswered => 'Odpowiedziałeś na wszystkie pytania';

  @override
  String get studyAllQuestionsAnsweredDescription => 'Dobra robota!';

  @override
  String get kidsNotificationReminderCtaShort => 'Turn on';

  @override
  String get kidsNotificationReminderCtaLong => 'Turn on reminders';

  @override
  String get kidsNotificationReminderTitle => 'Watch a video every day!';

  @override
  String get notifications => 'Notifications';

  @override
  String get onlyShowContentWithPreferredLanguages =>
      'Only show content with my preferred languages';
}
