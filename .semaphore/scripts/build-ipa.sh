...
#
# Relies on BUILD_NUMBER, APP_VERSION (and DART_VM_OPTIONS) being exported by the
# caller; child processes inherit exported env, so all are visible here.
# `ios/ExportOptions.plist` is resolved relative to the current directory, matching how
# the caller cd's in.
#
...
# Required, not optional: without --build-name flutter falls back to pubspec's version,
# and that is what kept shipping builds under an already-submitted train for TestFlight
# to reject (altool 90186/90062). A missing value must stop the build, not quietly pick
# the dead version.
if [ -z "${APP_VERSION:-}" ] || [ -z "${BUILD_NUMBER:-}" ]; then
  echo "ERROR: APP_VERSION and BUILD_NUMBER must be exported before calling build-ipa.sh" >&2
  exit 1
fi
...
until [ "$attempt" -ge 3 ]; do
  ...
  flutter build ipa --release $flavor_arg -t "$entrypoint" \
    ...
    --export-options-plist=ios/ExportOptions.plist && break
  attempt=$((attempt + 1))
